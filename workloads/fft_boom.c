// Algorithm from Department of Mathematics | Washington University in St. Louis
// https://www.math.wustl.edu/~victor/mfmm/fourier/fft.c

/* Factored discrete Fourier transform, or FFT, and its inverse iFFT */

#include "hpm_boom.h" // Adding include
#include <assert.h>
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
...
main(void)
{
  ...
  complex v_ref[N], v1_ref[N];
  int k;

  // Running these calculations early
  for(k=0; k<N; k++) {
      v_ref[k].Re = 0.125*cos(2*PI*k/(double)N);
      v_ref[k].Im = 0.125*sin(2*PI*k/(double)N);
      v1_ref[k].Re =  0.3*cos(2*PI*k/(double)N);
      v1_ref[k].Im = -0.3*sin(2*PI*k/(double)N);
  }
